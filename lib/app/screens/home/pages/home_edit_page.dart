...
class HomeEditCompanyPage extends StatefulWidget {
  final data;
  const HomeEditCompanyPage({super.key, required this.data});

  @override
  State<HomeEditCompanyPage> createState() => _HomeEditCompanyPageState();
}

class _HomeEditCompanyPageState extends State<HomeEditCompanyPage> {
  String selectedDay = '';
  String startTime = '';
  String finishTime = '';
  String stopStart = '';
  ...
  void initializeData() async {
    ...
    bin.text = widget.data['bin'] ?? '';
    mail.text = widget.data['mail'] ?? '';
    director.text = widget.data['director'] ?? '';
    ...
    finishTimeController.text = widget.data['finishTime'] ?? '';
    distance.text = widget.data['distance'] != null
        ? widget.data['distance'].toString()
        : '';
    estimate.text = widget.data['estimate'] != null
        ? widget.data['estimate'].toString()
        : '';

    price1.text =
        widget.data['price1'] != null ? widget.data['price1'].toString() : '';
    price2.text =
        widget.data['price2'] != null ? widget.data['price2'].toString() : '';
    price3.text =
        widget.data['price3'] != null ? widget.data['price3'].toString() : '';
    addedImages = widget.data['images'];
  }

  void loadData() {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      for (var i = 0; i < widget.data['routes'].length; i++) {
        var route = await ApiClient().get('routes', widget.data['routes'][i]);
        setState(() {
          routes.add(route);
        });
      }
